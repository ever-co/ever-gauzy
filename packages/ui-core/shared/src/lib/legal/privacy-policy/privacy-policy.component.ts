...
export class PrivacyPolicyComponent implements OnInit, OnDestroy {
	...
	getPrivacyPolicyJsonFromUrl(url: string) {
		this.legalService.getContentFromFromUrl(PRIVACY_POLICY_ENDPOINT).then((data: any) => {
			if (!!data.content) {
	...
	getCookiePolicyJsonFromUrl(url: string) {
		this.legalService.getContentFromFromUrl(COOKIE_PRIVACY_POLICY_ENDPOINT).then((data: any) => {
			if (!!data.content) {
