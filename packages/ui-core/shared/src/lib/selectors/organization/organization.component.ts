...
export class OrganizationSelectorComponent implements AfterViewInit, OnInit, OnDestroy {
	 ...
	 *
	 * `classes` is the element's STATIC class attribute, captured once via
	...
	get panelClass(): string | null {
		return this.dropdownClass ? `gauzy-entity-select organization-entity-select ${this.dropdownClass}` : null;
	}
