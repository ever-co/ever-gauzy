...
export class TranslateAdapterService implements IPluginTranslateService {
	...
	/** Synchronous translation lookup. Delegates to `TranslateService.instant()`. */
	instant(key: string, params?: Record<string, unknown>): string {
		return this._translate.instant(key, params);
	}

	/** Reactive translation. Re-emits when language or translations change. Delegates to `TranslateService.stream()`. */
	 ...
	 * Forwards `TranslateService.onLangChange` from `@ngx-translate/core`.
	 */
