...
@Component({
	...
	template: ` <gz-framework-host frameworkId="react" [component]="widget" [props]="{ title: 'Hello!' }" /> `
})
...
```html
<div [reactHost]="MyReactComponent" [props]="props"></div>
```

With extra context:

```html
<div [reactHost]="KanbanBoard" [props]="props" [context]="{ pageTitle: 'Kanban' }"></div>
```

#### 3. Render a React component (lazy / code-split)

```html
<div [reactLazyHost]="loadKanban" [props]="kanbanProps" [context]="{ pageTitle: 'Kanban' }"></div>
```
...
| Export                   | Description                                                  |
| ------------------------ | ------------------------------------------------------------ |
| `ReactHostDirective`     | Directive `[reactHost]` – eager render                       |
| `LazyReactHostDirective` | Directive `[reactLazyHost]` – lazy render via dynamic import |
