...
export async function up(knex: Knex): Promise<void> {
	await knex.schema.alterTable(TABLE_NAME_TASKS, (table: Knex.TableBuilder) => {
		table.integer('tags').unsigned().index().references('id').inTable(TABLE_NAME_TAGS).onDelete('SET NULL');
		table.integer('members').unsigned().index().references('id').inTable(TABLE_NAME_USERS).onDelete('SET NULL');
	});
}
