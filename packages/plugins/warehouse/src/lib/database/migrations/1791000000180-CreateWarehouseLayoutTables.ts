...
export class CreateWarehouseLayoutTables1791000000180 implements MigrationInterface {
	...
	public async postgresUpQueryRunner(queryRunner: QueryRunner): Promise<any> {
		...
		await queryRunner.query(
			`CREATE UNIQUE INDEX "UQ_warehouse_zone_code" ON "warehouse_zone" ("organizationId", "warehouseId", "code") WHERE "deletedAt" IS NULL`
		);
		...
		await queryRunner.query(
			`CREATE UNIQUE INDEX "UQ_warehouse_bin_code" ON "warehouse_bin" ("organizationId", "warehouseId", "code") WHERE "deletedAt" IS NULL`
		);
		await queryRunner.query(
			`CREATE UNIQUE INDEX "UQ_warehouse_bin_barcode" ON "warehouse_bin" ("organizationId", "barcode") WHERE "barcode" IS NOT NULL AND "deletedAt" IS NULL`
		);
		await queryRunner.query(
	...
	public async mysqlUpQueryRunner(queryRunner: QueryRunner): Promise<any> {
		await queryRunner.query(
			`CREATE TABLE \`warehouse_zone\` (\`deletedAt\` datetime(6) NULL, \`createdAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updatedAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`createdByUserId\` varchar(36) NULL, \`updatedByUserId\` varchar(36) NULL, \`deletedByUserId\` varchar(36) NULL, \`id\` varchar(36) NOT NULL, \`isActive\` tinyint NULL DEFAULT 1, \`isArchived\` tinyint NULL DEFAULT 0, \`archivedAt\` datetime NULL, \`tenantId\` varchar(36) NULL, \`organizationId\` varchar(36) NULL, \`warehouseId\` varchar(36) NOT NULL, \`name\` varchar(255) NOT NULL, \`code\` varchar(64) NOT NULL, \`type\` varchar(16) NOT NULL DEFAULT 'STORAGE', \`priority\` int NOT NULL DEFAULT 0, \`isPickable\` tinyint NOT NULL DEFAULT 1, \`isReceivable\` tinyint NOT NULL DEFAULT 0, \`isShippable\` tinyint NOT NULL DEFAULT 0, \`isBlocked\` tinyint NOT NULL DEFAULT 0, \`minTemperature\` decimal(6,2) NULL, \`maxTemperature\` decimal(6,2) NULL, \`version\` int NOT NULL DEFAULT 1, \`metadata\` json NULL, INDEX \`IDX_warehouse_zone_created_by_user\` (\`createdByUserId\`), INDEX \`IDX_warehouse_zone_updated_by_user\` (\`updatedByUserId\`), INDEX \`IDX_warehouse_zone_deleted_by_user\` (\`deletedByUserId\`), INDEX \`IDX_warehouse_zone_is_active\` (\`isActive\`), INDEX \`IDX_warehouse_zone_is_archived\` (\`isArchived\`), INDEX \`IDX_warehouse_zone_tenant\` (\`tenantId\`), INDEX \`IDX_warehouse_zone_organization\` (\`organizationId\`), INDEX \`IDX_warehouse_zone_warehouse_type\` (\`warehouseId\`, \`type\`, \`priority\`), INDEX \`IDX_warehouse_zone_pickable\` (\`warehouseId\`, \`isPickable\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`
		);
		// MySQL has no partial index, so the uniqueness a deleted row must not consume is expressed by
		// carrying `deletedAt` in the key: two live rows can never collide, and a soft-deleted row keeps
		// its own value.
		await queryRunner.query(
			`CREATE UNIQUE INDEX \`UQ_warehouse_zone_code\` ON \`warehouse_zone\` (\`organizationId\`, \`warehouseId\`, \`code\`, \`deletedAt\`)`
		);
		await queryRunner.query(
