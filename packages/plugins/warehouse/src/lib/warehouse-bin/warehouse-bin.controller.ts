...
export class WarehouseBinController extends CrudController<WarehouseBin> {
	 ...
	 * this controller's class-level read grant alone. It now states `WAREHOUSE_BINS_DELETE`, the grant the
	 * plugin's own `deleteWarehouseBin` mutation carries, so both surfaces ask the same caller.
	 *
	 * @param id The bin to soft delete.
	 * @returns The soft-deleted bin.
	 */
	@ApiOperation({ summary: 'Soft delete a bin' })
	@ApiResponse({ status: HttpStatus.ACCEPTED, description: 'The bin was soft deleted.' })
	@Permissions(WarehousePermissions.WAREHOUSE_BINS_DELETE)
	@Delete(':id/soft')
	@HttpCode(HttpStatus.ACCEPTED)
	 ...
	 * `PermissionGuard` (`packages/core/src/lib/shared/guards/permission.guard.ts`) answers `true` to that
	 * empty metadata with its `isEmpty(permissions)` return, which left the inherited handler standing on
	 * this controller's class-level read grant alone. It now states `WAREHOUSE_BINS_DELETE` — restoring is
	 * the same grant exercised backwards, exactly as the delete route above states it.
	 *
	 * @param id The bin to restore.
	 * @returns The restored bin.
	 */
	@ApiOperation({ summary: 'Restore a soft-deleted bin' })
	@ApiResponse({ status: HttpStatus.ACCEPTED, description: 'The bin was restored.' })
	@Permissions(WarehousePermissions.WAREHOUSE_BINS_DELETE)
	@Put(':id/recover')
	@HttpCode(HttpStatus.ACCEPTED)
	@UsePipes(new AbstractValidationPipe({ whitelist: true }, { query: TenantOrganizationBaseDTO }))
