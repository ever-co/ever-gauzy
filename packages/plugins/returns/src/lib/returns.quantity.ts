...
/**
 * Exact quantity arithmetic for the returns domain.
 *
 * Quantities are stored as `numeric(20,6)` and read back as decimal strings, so comparing them as
 * floating point numbers would answer "did this return exceed what was shipped?" with a rounding
 * error near the boundary — exactly where the answer matters. The helpers below scale a decimal
 * string into an exact integer (`bigint`) at the storage scale and do every comparison there, so a
 * ceiling check is exact regardless of how the quantity was written.
 *
 * This is deliberately not the money layer: a quantity is a count of units, not an amount of money,
 * and it has no currency, no rounding strategy and no minor units to honour.
 */

/** Fractional digits a quantity column carries. */
export const QUANTITY_SCALE = 6;

/** Ten raised to the storage scale, as the factor that lifts a decimal into whole units. */
