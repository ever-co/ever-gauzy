 ...
 * partial indexes, so every uniqueness that the specification states as a predicate is expressed over a
 * stored `deletedKey` column (and, where the predicate is a null guard, over the nullable column itself,
 * because MySQL treats NULLs as distinct) — the writing service re-checks the predicate, so the
 * guarantee is never weaker than the service that relies on it.
 */
export class CreateMarketplaceTables1791000000380 implements MigrationInterface {
	...
	public async postgresUpQueryRunner(queryRunner: QueryRunner): Promise<any> {
		...
		await queryRunner.query(
			`CREATE UNIQUE INDEX "UQ_seller_org_code" ON "seller" ("organizationId", "code") WHERE "deletedAt" IS NULL`
		);
		await queryRunner.query(
			`CREATE UNIQUE INDEX "UQ_seller_org_contact" ON "seller" ("organizationId", "contactId") WHERE "deletedAt" IS NULL`
		);
		await queryRunner.query(
			`CREATE UNIQUE INDEX "UQ_seller_org_external" ON "seller" ("organizationId", "externalId") WHERE "externalId" IS NOT NULL AND "deletedAt" IS NULL`
		);
		await queryRunner.query(
		...
		await queryRunner.query(
			`CREATE UNIQUE INDEX "UQ_seller_payout_number" ON "seller_payout" ("organizationId", "number") WHERE "deletedAt" IS NULL`
		);
		await queryRunner.query(
			`CREATE UNIQUE INDEX "UQ_seller_payout_providerref" ON "seller_payout" ("providerKey", "providerTransferId") WHERE "providerTransferId" IS NOT NULL AND "deletedAt" IS NULL`
		);
	...
	public async mysqlUpQueryRunner(queryRunner: QueryRunner): Promise<any> {
		...
		await queryRunner.query(
			`CREATE UNIQUE INDEX \`UQ_seller_tx_sale\` ON \`seller_transaction\` (\`orderLineId\`, \`kind\`, \`deletedKey\`)`
		);
		await queryRunner.query(
