...
const perform = async (z: ZObject, bundle: Bundle) => {
  try {
    ...
    throw new z.errors.HaltedError(`Failed to start timer: ${error.message || 'Unknown error'}`);
  }
};
...
export default {
  ...
  display: {
    label: 'Start Timer',
    description: `Starts a timer in ${appName}.`
  },
  operation: {
    inputFields: [
      {
    ...
    sample: {
      ...
      logType: 'TRACKED',
      source: 'BROWSER',
      description: 'Timer started via Zapier integration',
      isBillable: false,
      version: '1.0.1',
