...
export class IntegrationEverAsyncLayoutComponent implements OnInit {
	...
	private initializeUiLanguagesAndLocale() {
		const preferredLanguage$ = merge(this._store.preferredLanguage$, this._i18nService.preferredLanguage$).pipe(
			distinctUntilChange(),
			filter(
				(lang: string | LanguagesEnum) => !!lang && Object.values(LanguagesEnum).includes(lang as LanguagesEnum)
			),
