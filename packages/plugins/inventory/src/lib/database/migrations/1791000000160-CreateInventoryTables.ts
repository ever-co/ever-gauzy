...
export class CreateInventoryTables1791000000160 implements MigrationInterface {
	...
	public async postgresUpQueryRunner(queryRunner: QueryRunner): Promise<any> {
		...
		await queryRunner.query(`CREATE UNIQUE INDEX "UQ_stock_transfer_number" ON "stock_transfer" (COALESCE("organizationId", \'00000000-0000-0000-0000-000000000000\'), "number")`);
		...
		await queryRunner.query(`CREATE UNIQUE INDEX "UQ_stock_alert" ON "stock_alert" ("variantId", COALESCE("warehouseId", \'00000000-0000-0000-0000-000000000000\'))`);
		...
		await queryRunner.query(`CREATE UNIQUE INDEX "UQ_stock_adjustment_number" ON "stock_adjustment" (COALESCE("organizationId", \'00000000-0000-0000-0000-000000000000\'), "number")`);
		...
		await queryRunner.query(`CREATE UNIQUE INDEX "UQ_stock_count_number" ON "stock_count" (COALESCE("organizationId", \'00000000-0000-0000-0000-000000000000\'), "number")`);
...
$$ LANGUAGE plpgsql`);
	 ...
	 * MySQL Up Migration
	 *
	 * Note what this dialect does **not** need here. The unique indexes of this file carry no
	 * `"deletedAt" IS NULL` predicate on Postgres or SQLite — a soft-deleted transfer keeps its number —
	 * so MySQL must not carry a `deletedKey` either: adding one would exempt every deleted row and make
	 * this dialect the lenient one. Naming `deletedAt` itself in the tuple, which this file used to do,
	 * did exactly that and more, because a unique index in MySQL exempts every tuple that contains a
	 * null and a live row's `deletedAt` is null.
	 *
	 * What it does need is the two nullable scope columns folded, in the form
	 * `CreateSequenceTable1791000000000` documents: `organizationKey` for the numbering rules, and
	 * `warehouseKey` for a stock alert that is raised across every warehouse rather than in one.
	 *
	 * @param queryRunner
	 */
	public async mysqlUpQueryRunner(queryRunner: QueryRunner): Promise<any> {
		...
		await queryRunner.query(`CREATE TABLE \`stock_transfer\` (\`deletedAt\` datetime(6) NULL, \`createdAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updatedAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`createdByUserId\` varchar(36) NULL, \`updatedByUserId\` varchar(36) NULL, \`deletedByUserId\` varchar(36) NULL, \`id\` varchar(36) NOT NULL, \`isActive\` tinyint NULL DEFAULT 1, \`isArchived\` tinyint NULL DEFAULT 0, \`archivedAt\` datetime NULL, \`tenantId\` varchar(36) NULL, \`organizationId\` varchar(36) NULL, \`number\` varchar(64) NOT NULL, \`fromWarehouseId\` varchar(36) NOT NULL, \`toWarehouseId\` varchar(36) NOT NULL, \`status\` varchar(32) NOT NULL DEFAULT 'DRAFT', \`shippedAt\` datetime, \`receivedAt\` datetime, \`note\` text, \`version\` int NOT NULL DEFAULT 1, \`metadata\` json, CONSTRAINT \`CHK_stock_transfer_distinct\` CHECK (\`fromWarehouseId\` <> \`toWarehouseId\`), \`organizationKey\` varchar(36) GENERATED ALWAYS AS (IFNULL(\`organizationId\`, \'00000000-0000-0000-0000-000000000000\')) STORED, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
		await queryRunner.query(`CREATE UNIQUE INDEX \`UQ_stock_transfer_number\` ON \`stock_transfer\` (\`organizationKey\`, \`number\`)`);
		...
		await queryRunner.query(`CREATE UNIQUE INDEX \`UQ_stock_transfer_line\` ON \`stock_transfer_line\` (\`transferId\`, \`variantId\`)`);
		await queryRunner.query(`CREATE INDEX \`IDX_stock_transfer_line_variant\` ON \`stock_transfer_line\` (\`variantId\`)`);
		await queryRunner.query(`CREATE TABLE \`stock_alert\` (\`deletedAt\` datetime(6) NULL, \`createdAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updatedAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`createdByUserId\` varchar(36) NULL, \`updatedByUserId\` varchar(36) NULL, \`deletedByUserId\` varchar(36) NULL, \`id\` varchar(36) NOT NULL, \`isActive\` tinyint NULL DEFAULT 1, \`isArchived\` tinyint NULL DEFAULT 0, \`archivedAt\` datetime NULL, \`tenantId\` varchar(36) NULL, \`organizationId\` varchar(36) NULL, \`variantId\` varchar(36) NOT NULL, \`warehouseId\` varchar(36), \`threshold\` decimal(20, 6) NOT NULL, \`notifyEmails\` text, \`notifyRoles\` text, \`lastTriggeredAt\` datetime, \`cooldownMinutes\` int NOT NULL DEFAULT 0, \`warehouseKey\` varchar(36) GENERATED ALWAYS AS (IFNULL(\`warehouseId\`, \'00000000-0000-0000-0000-000000000000\')) STORED, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
		await queryRunner.query(`CREATE UNIQUE INDEX \`UQ_stock_alert\` ON \`stock_alert\` (\`variantId\`, \`warehouseKey\`)`);
