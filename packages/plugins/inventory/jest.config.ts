module.exports = {
	...
	// context, `camelcase`, …). Without this exception the suite fails to LOAD and Jest reports "0 tests
	// in 1 suite" instead of failing an assertion. Same list, and the same reasoning, as
	// `packages/core/jest.config.ts`; the exception only does anything because `allowJs` is true in
	// `tsconfig.spec.json`.
	...
	coverageDirectory: '../../../coverage/packages/plugins/inventory'
};
