...
export class OrderChangeActionController extends CrudController<OrderChangeAction> {
	...
	/**
	 * Deletes an order change action by id.
	 *
	 ...
	 *
	 * @param id The order change action.
	 * @returns The result of the delete.
	 */
	@ApiOperation({ summary: 'Delete an order change action' })
	@ApiResponse({ status: HttpStatus.ACCEPTED, description: 'The order change action was deleted' })
	@Permissions(ORDER_PERMISSIONS.ORDERS_EDIT)
	@Delete(':id')
	@HttpCode(HttpStatus.ACCEPTED)
	...
	/**
	 * Soft deletes an order change action by id.
	 *
	 ...
	 *
	 * @param id The order change action.
	 * @returns The soft-deleted order change action.
	 */
	@ApiOperation({ summary: 'Soft delete an order change action' })
	@ApiResponse({ status: HttpStatus.ACCEPTED, description: 'The order change action was soft deleted' })
	@Permissions(ORDER_PERMISSIONS.ORDERS_EDIT)
	@Delete(':id/soft')
	@HttpCode(HttpStatus.ACCEPTED)
	...
	/**
	 * Restores a soft-deleted order change action by id.
	 *
	 ...
	 *
	 * @param id The order change action.
	 * @returns The restored order change action.
	 */
	@ApiOperation({ summary: 'Restore a soft-deleted order change action' })
	@ApiResponse({ status: HttpStatus.ACCEPTED, description: 'The order change action was restored' })
	@Permissions(ORDER_PERMISSIONS.ORDERS_EDIT)
	@Put(':id/recover')
	@HttpCode(HttpStatus.ACCEPTED)
	@UsePipes(new AbstractValidationPipe({ whitelist: true }, { query: TenantOrganizationBaseDTO }))
