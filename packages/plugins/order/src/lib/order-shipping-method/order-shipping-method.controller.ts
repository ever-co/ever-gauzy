...
export class OrderShippingMethodController extends CrudController<OrderShippingMethod> {
	...
	/**
	 * Deletes an order shipping method by id.
	 *
	 ...
	 *
	 * @param id The order shipping method.
	 * @returns The result of the delete.
	 */
	@ApiOperation({ summary: 'Delete an order shipping method' })
	@ApiResponse({ status: HttpStatus.ACCEPTED, description: 'The order shipping method was deleted' })
	@Permissions(ORDER_PERMISSIONS.ORDERS_EDIT)
	@Delete(':id')
	@HttpCode(HttpStatus.ACCEPTED)
	...
	/**
	 * Soft deletes an order shipping method by id.
	 *
	 ...
	 *
	 * @param id The order shipping method.
	 * @returns The soft-deleted order shipping method.
	 */
	@ApiOperation({ summary: 'Soft delete an order shipping method' })
	@ApiResponse({ status: HttpStatus.ACCEPTED, description: 'The order shipping method was soft deleted' })
	@Permissions(ORDER_PERMISSIONS.ORDERS_EDIT)
	@Delete(':id/soft')
	@HttpCode(HttpStatus.ACCEPTED)
	...
	/**
	 * Restores a soft-deleted order shipping method by id.
	 *
	 ...
	 *
	 * @param id The order shipping method.
	 * @returns The restored order shipping method.
	 */
	@ApiOperation({ summary: 'Restore a soft-deleted order shipping method' })
	@ApiResponse({ status: HttpStatus.ACCEPTED, description: 'The order shipping method was restored' })
	@Permissions(ORDER_PERMISSIONS.ORDERS_EDIT)
	@Put(':id/recover')
	@HttpCode(HttpStatus.ACCEPTED)
	@UsePipes(new AbstractValidationPipe({ whitelist: true }, { query: TenantOrganizationBaseDTO }))
