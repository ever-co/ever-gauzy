...
import { CreateOrderCreditLineDTO, UpdateOrderCreditLineDTO } from './dto';
...
export class OrderCreditLineController extends CrudController<OrderCreditLine> {
	 ...
	 */
	@ApiOperation({ summary: 'Create an order credit line' })
	@ApiResponse({ status: HttpStatus.CREATED, description: 'The order credit line was created', type: OrderCreditLine })
	@Permissions(ORDER_PERMISSIONS.ORDERS_CREATE)
	@HttpCode(HttpStatus.CREATED)
	@Post()
	@UseValidationPipe({ transform: true, whitelist: true })
	async create(@Body() entity: CreateOrderCreditLineDTO): Promise<OrderCreditLine> {
		return this.service.create(entity as any);
	}

	/**
	 * Updates an order credit line.
	 *
	 ...
	 */
	@ApiOperation({ summary: 'Update an order credit line' })
	@ApiResponse({ status: HttpStatus.ACCEPTED, description: 'The order credit line was updated', type: OrderCreditLine })
	...
	async update(
		...
		@Body() entity: UpdateOrderCreditLineDTO
	): Promise<any> {
		return this.service.update(id, entity as any);
	}

	/**
	 * Deletes an order credit line by id.
	 *
	 ...
	 *
	 * @param id The order credit line.
	 * @returns The result of the delete.
	 */
	@ApiOperation({ summary: 'Delete an order credit line' })
	@ApiResponse({ status: HttpStatus.ACCEPTED, description: 'The order credit line was deleted' })
	@Permissions(ORDER_PERMISSIONS.ORDERS_EDIT)
	@Delete(':id')
	@HttpCode(HttpStatus.ACCEPTED)
	...
	/**
	 * Soft deletes an order credit line by id.
	 *
	 ...
	 *
	 * @param id The order credit line.
	 * @returns The soft-deleted order credit line.
	 */
	@ApiOperation({ summary: 'Soft delete an order credit line' })
	@ApiResponse({ status: HttpStatus.ACCEPTED, description: 'The order credit line was soft deleted' })
	@Permissions(ORDER_PERMISSIONS.ORDERS_EDIT)
	@Delete(':id/soft')
	@HttpCode(HttpStatus.ACCEPTED)
	...
	/**
	 * Restores a soft-deleted order credit line by id.
	 *
	 ...
	 *
	 * @param id The order credit line.
	 * @returns The restored order credit line.
	 */
	@ApiOperation({ summary: 'Restore a soft-deleted order credit line' })
	@ApiResponse({ status: HttpStatus.ACCEPTED, description: 'The order credit line was restored' })
	@Permissions(ORDER_PERMISSIONS.ORDERS_EDIT)
	@Put(':id/recover')
	@HttpCode(HttpStatus.ACCEPTED)
	@UsePipes(new AbstractValidationPipe({ whitelist: true }, { query: TenantOrganizationBaseDTO }))
