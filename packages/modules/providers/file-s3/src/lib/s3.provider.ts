...
import { trimIfNotEmpty, parseToBoolean, ensureHttpPrefix } from '@gauzy/utils';
import { Provider } from './provider';
import { IS3Config, IS3ConfigProvider } from './interfaces';
import { S3_CONFIG, S3_CONFIG_PROVIDER, DEFAULT_S3_REGION } from './constants';
 ...
 *
 * A flexible and reusable storage provider for AWS S3 cloud storage.
...
export class S3Provider extends Provider<S3Provider> {
	/**
	 * Provider name matching FileStorageProviderEnum.S3
	 ...
	 */
	public config: FileSystem & IS3FileStorageProviderConfig;

	private _staticConfig: IS3Config | null = null;
	private _configProvider: IS3ConfigProvider | null = null;
	private _detailedLoggingEnabled = false;

	constructor(
		@Optional() @Inject(S3_CONFIG) staticConfig?: IS3Config,
		@Optional() @Inject(S3_CONFIG_PROVIDER) configProvider?: IS3ConfigProvider
	) {
		super();
	 ...
	 */
	public setConfig(config: IS3Config): void {
	 ...
	 */
	public setConfigProvider(provider: IS3ConfigProvider): void {
	 ...
	 */
	public getProviderInstance(): S3Provider {
		if (!this.instance) {
			this.instance = this;
		}
		...
		try {
			...
			return multerS3({
				...
				bucket: (_req, _file, callback) => {
					callback(null, this.getS3Bucket());
				},
	...
	/**
	 * Create an empty configuration object matching FileSystem & IS3FileStorageProviderConfig
	 */
	private _createEmptyConfig(): FileSystem & IS3FileStorageProviderConfig {
		return {
			rootPath: '',
			aws_access_key_id: '',
	...
	private _refreshConfig(): void {
		...
		let rawConfig: IS3Config | null = null;
		...
		if (rawConfig) {
			// Map to FileSystem & IS3FileStorageProviderConfig format for core compatibility
			this.config = {
				rootPath: rawConfig.rootPath ?? '',
				aws_access_key_id: trimIfNotEmpty(rawConfig.accessKeyId) ?? '',
				aws_secret_access_key: trimIfNotEmpty(rawConfig.secretAccessKey) ?? '',
				aws_default_region: rawConfig.region ?? DEFAULT_S3_REGION,
				aws_bucket: trimIfNotEmpty(rawConfig.bucket) ?? '',
				aws_force_path_style: parseToBoolean(rawConfig.forcePathStyle)
			};
		}
	...
	private _getS3Client(): S3Client | null {
		...
		const { aws_access_key_id, aws_secret_access_key, aws_default_region, aws_force_path_style } = this.config;

		if (!aws_access_key_id || !aws_secret_access_key) {
			this._log('warn', 'Incomplete S3 configuration - missing credentials');
			return null;
		}
		...
		try {
			const s3ClientConfig: any = {
				...
				region: aws_default_region || DEFAULT_S3_REGION,
				forcePathStyle: aws_force_path_style
			};

			// Check for custom service URL (for S3-compatible storage)
			const serviceUrl = this._configProvider?.getConfig()?.serviceUrl || this._staticConfig?.serviceUrl;
			if (serviceUrl) {
				s3ClientConfig.endpoint = ensureHttpPrefix(serviceUrl);
			}
