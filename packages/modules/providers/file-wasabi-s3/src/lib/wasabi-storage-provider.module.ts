...
import { IWasabiConfig, IWasabiStorageModuleOptions, IWasabiStorageModuleAsyncOptions } from './interfaces';
import { WASABI_CONFIG, WASABI_CONFIG_PROVIDER, WASABI_S3_PROVIDER } from './constants';

/**
 * Wasabi Storage Provider Module
 *
 ...
 * WasabiStorageProviderModule.registerAsync({
 ...
 */
@Module({})
export class WasabiStorageProviderModule {
	/**
	...
	static register(options: IWasabiStorageModuleOptions = {}): DynamicModule {
		...
		return {
			module: WasabiStorageProviderModule,
			global: isGlobal,
			providers,
...
export function createConfigFromEnv(envPrefix: string = 'WASABI'): IWasabiConfig {
	return {
		...
		// In Gauzy, the bucket env is `${envPrefix}_S3_BUCKET` (e.g., WASABI_S3_BUCKET)
