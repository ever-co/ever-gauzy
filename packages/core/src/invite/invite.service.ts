...
export class InviteService extends TenantAwareCrudService<Invite> {
	...
	async acceptMyInvitation(body: AcceptMyInviteDTO, origin: string, languageCode: LanguagesEnum) {
		const user = RequestContext.currentUser();
		const query = this.repository.createQueryBuilder(this.alias);
		...
		query.andWhere((qb: SelectQueryBuilder<Invite>) => {
			...
			subQuery
				.select('"invite_organization_team"."organizationTeamId"')
				.from('invite_organization_team', 'invite_organization_team');
