...
import { OnPluginBootstrap, OnPluginDestroy, PluginMetaData } from '@gauzy/common';
import { PLUGIN_METADATA } from '../constants';

export function ExtensionPlugin(pluginMetadata: PluginMetaData): ClassDecorator {
  return (target) => {
    ...
    const metadata = _.pick(pluginMetadata, Object.values(MODULE_METADATA) as string[]);
