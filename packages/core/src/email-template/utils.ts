...
export class EmailTemplateUtils {
	 ...
	 * @param folder
	 */
	public static async migrateEmailTemplates(queryRunner: QueryRunner, folder: EmailTemplateNameEnum): Promise<void> {
		...
		if (EmailTemplateUtils.fileExists(templatePath)) {
			if (isNotEmpty(folder)) {
				...
				console.log(
					chalk.magenta(
						`${moment().format('DD.MM.YYYY HH:mm:ss')} Migrated email templates for ${folderPath}`
					)
				);
			}
		}
	 ...
	 * @param templates
	 */
	public static async createOrUpdateTemplates(queryRunner: QueryRunner, templates: Array<Object> = []) {
