...
export class StatisticService {
	...
	async getCounts(request: IGetCountsStatistics): Promise<ICountsStatistics> {
		const { organizationId, startDate, endDate, todayStart, todayEnd } =
			request;
		...
		const weekTimeStatistics = await weekQuery
			...
			.addSelect(
				`COALESCE(SUM("${weekQuery.alias}"."overall"), 0)`,
				`overall`
			)
			.addSelect(
				`COALESCE(SUM("${weekQuery.alias}"."duration"), 0)`,
				`duration`
			)
			.addSelect(`COUNT("${weekQuery.alias}"."id")`, `time_slot_count`)
			.andWhere(`"${weekQuery.alias}"."tenantId" = :tenantId`, {
				tenantId,
			})
			.andWhere(
				`"${weekQuery.alias}"."organizationId" = :organizationId`,
				{ organizationId }
			)
			.andWhere(
				new Brackets((qb: WhereExpressionBuilder) => {
					qb.andWhere(
						`"timeLogs"."startedAt" BETWEEN :startDate AND :endDate`,
						{
							startDate: start,
							endDate: end,
						}
					...
					qb.andWhere(
						`"${weekQuery.alias}"."startedAt" BETWEEN :startDate AND :endDate`,
						{
							startDate: start,
							endDate: end,
						}
					...
					if (isNotEmpty(employeeIds)) {
						qb.andWhere(
							`"${weekQuery.alias}"."employeeId" IN (:...employeeIds)`,
							{
								employeeIds,
							}
						);
					...
					if (isNotEmpty(request.activityLevel)) {
						...
						qb.andWhere(
							`"${weekQuery.alias}"."overall" BETWEEN :startLevel AND :endLevel`,
							{
								startLevel,
								endLevel,
							}
					...
					qb.andWhere(
						new Brackets((qb: WhereExpressionBuilder) => {
							qb.andWhere(`"timeLogs"."tenantId" = :tenantId`, {
								tenantId,
							});
							qb.andWhere(
								`"timeLogs"."organizationId" = :organizationId`,
								{ organizationId }
							);
		...
		const weekDuration = reduce(
			pluck(weekTimeStatistics, 'week_duration'),
			ArraySum,
			0
		);
		...
		const todayTimeStatistics = await todayQuery
			...
			.addSelect(
				`COALESCE(SUM("${todayQuery.alias}"."overall"), 0)`,
				`overall`
			)
			.addSelect(
				`COALESCE(SUM("${todayQuery.alias}"."duration"), 0)`,
				`duration`
			)
			.addSelect(`COUNT("${todayQuery.alias}"."id")`, `time_slot_count`)
			.andWhere(`"${todayQuery.alias}"."tenantId" = :tenantId`, {
				tenantId,
			})
			.andWhere(
			...
			.andWhere(
				new Brackets((qb: WhereExpressionBuilder) => {
					...
					qb.andWhere(
						`"${todayQuery.alias}"."startedAt" BETWEEN :startDate AND :endDate`,
						{
							startDate: startToday,
							endDate: endToday,
						}
					...
					if (isNotEmpty(request.activityLevel)) {
						...
						qb.andWhere(
							`"${todayQuery.alias}"."overall" BETWEEN :startLevel AND :endLevel`,
							{
								startLevel,
								endLevel,
							}
		...
		const todayDuration = reduce(
			pluck(todayTimeStatistics, 'today_duration'),
			ArraySum,
			0
		);
	...
	async getMembers(
		...
		let employees: IMembersStatistics[] = await query
			...
			.andWhere(
				new Brackets((qb: WhereExpressionBuilder) => {
					qb.andWhere(
						`"timeLogs"."startedAt" BETWEEN :weeklyStart AND :weeklyEnd`,
						{
							weeklyStart,
							weeklyEnd,
						}
					...
					qb.andWhere(
						`"time_slot"."startedAt" BETWEEN :weeklyStart AND :weeklyEnd`,
						{
							weeklyStart,
							weeklyEnd,
						}
		...
		if (employees.length > 0) {
			 ...
			 */
			const weekTimeQuery =
				this.timeSlotRepository.createQueryBuilder('time_slot');
			weekTimeQuery
				.select(
					`${
						this.configService.dbConnectionOptions.type === 'sqlite'
				...
				.addSelect(
					`COALESCE(SUM("${weekTimeQuery.alias}"."overall"), 0)`,
					`overall`
				)
				.addSelect(
					`COALESCE(SUM("${weekTimeQuery.alias}"."duration"), 0)`,
					`duration`
				)
				.addSelect(
					`COUNT("${weekTimeQuery.alias}"."id")`,
					`time_slot_count`
				)
				...
				.andWhere(
					new Brackets((qb: WhereExpressionBuilder) => {
						...
						qb.andWhere(
							`"${weekTimeQuery.alias}"."startedAt" BETWEEN :weeklyStart AND :weeklyEnd`,
							{
								weeklyStart,
								weeklyEnd,
							}
			...
			dayTimeQuery
				...
				.addSelect(
					`COALESCE(SUM("${dayTimeQuery.alias}"."overall"), 0)`,
					`overall`
				)
				.addSelect(
					`COALESCE(SUM("${dayTimeQuery.alias}"."duration"), 0)`,
					`duration`
				)
				.addSelect(
					`COUNT("${dayTimeQuery.alias}"."id")`,
					`time_slot_count`
				)
				...
				.andWhere(
					new Brackets((qb: WhereExpressionBuilder) => {
						...
						qb.where(
							`"timeLogs"."startedAt" BETWEEN :startToday AND :endToday`,
							{
								startToday,
								endToday,
							}
						...
						qb.andWhere(
							`"${dayTimeQuery.alias}"."startedAt" BETWEEN :startToday AND :endToday`,
							{
								startToday,
								endToday,
							}
			...
			for (let index = 0; index < employees.length; index++) {
				...
				weekHoursQuery
					...
					.andWhere(`"${weekHoursQuery.alias}"."id" = :memberId`, {
						memberId: member.id,
					})
					.andWhere(
					...
					.andWhere(
						`"${weekHoursQuery.alias}"."organizationId" = :organizationId`,
						{ organizationId }
					)
					.andWhere(
						new Brackets((qb: WhereExpressionBuilder) => {
	 ...
	 */
	async getProjects(
		request: IGetProjectsStatistics
		...
		query
			...
			.andWhere(
				new Brackets((qb: WhereExpressionBuilder) => {
					qb.andWhere(
						`"${query.alias}"."startedAt" BETWEEN :start AND :end`,
						{
							start,
							end,
						}
					...
					qb.andWhere(
						`"time_slot"."startedAt" BETWEEN :start AND :end`,
						{
							start,
							end,
						}
		...
		const totalDurationQuery =
			this.timeLogRepository.createQueryBuilder('time_log');
		totalDurationQuery
			.select(
				`${
					this.configService.dbConnectionOptions.type === 'sqlite'
			...
			.andWhere(
				new Brackets((qb: WhereExpressionBuilder) => {
					qb.andWhere(
						`"${totalDurationQuery.alias}"."startedAt" BETWEEN :start AND :end`,
						{
							start,
							end,
						}
			...
			.andWhere(
				new Brackets((qb: WhereExpressionBuilder) => {
					...
					qb.andWhere(
						`"${totalDurationQuery.alias}"."deletedAt" IS NULL`
					);
				})
			)
			.andWhere(
	...
	async getTasks(request: IGetTasksStatistics) {
		...
		let dayStart: string | Date;
		let dayEnd: string | Date;
	 ...
	 */
	async manualTimes(
		request: IGetManualTimesStatistics
		...
		query.where((qb: SelectQueryBuilder<TimeLog>) => {
			qb.andWhere(
				new Brackets((web: WhereExpressionBuilder) => {
					...
					web.andWhere(
						`"time_slots"."startedAt" BETWEEN :start AND :end`,
						{
							start,
							end,
						}
			...
			qb.andWhere(
				new Brackets((web: WhereExpressionBuilder) => {
					if (isNotEmpty(employeeIds)) {
						web.andWhere(
							`"${qb.alias}"."employeeId" IN (:...employeeIds)`,
							{
								employeeIds,
							}
						);
	 ...
	 */
	async getActivities(
		request: IGetActivitiesStatistics
		...
		query
			...
			.andWhere(
				new Brackets((qb: WhereExpressionBuilder) => {
					qb.andWhere(
						`"time_log"."startedAt" BETWEEN :startDate AND :endDate`,
						{
							startDate: start,
							endDate: end,
						}
					...
					qb.andWhere(
						`"time_slot"."startedAt" BETWEEN :startDate AND :endDate`,
						{
							startDate: start,
							endDate: end,
						}
			...
			.andWhere(
				new Brackets((qb: WhereExpressionBuilder) => {
					qb.andWhere(`"time_log"."tenantId" = :tenantId`, {
						tenantId,
					});
					qb.andWhere(
	...
	async getEmployeeTimeSlots(
		request: IGetTimeSlotStatistics
			...
			query.where((qb: SelectQueryBuilder<TimeSlot>) => {
				...
				qb.andWhere(
					new Brackets((web: WhereExpressionBuilder) => {
						...
						web.andWhere(`"timeLogs"."tenantId" = :tenantId`, {
							tenantId,
						});
						web.andWhere(
							`"timeLogs"."organizationId" = :organizationId`,
	...
	private getFilterQuery(
		...
		qb.andWhere(
			new Brackets((qb: WhereExpressionBuilder) => {
				qb.andWhere(
					`"${query.alias}"."startedAt" BETWEEN :startDate AND :endDate`,
					{
						startDate: start,
						endDate: end,
					}
		...
		qb.andWhere(
			new Brackets((qb: WhereExpressionBuilder) => {
				if (isNotEmpty(request.activityLevel)) {
					...
					qb.andWhere(
						`"time_slot"."overall" BETWEEN :startLevel AND :endLevel`,
						{
							startLevel,
							endLevel,
						}
