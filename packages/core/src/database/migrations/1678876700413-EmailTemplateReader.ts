...
export class EmailTemplateReader1678876700413 implements MigrationInterface {
	...
	public async up(queryRunner: QueryRunner): Promise<any> {
		try {
			await EmailTemplateUtils.migrateEmailTemplates(queryRunner, EmailTemplateNameEnum.EMAIL_RESET);
		} catch (error) {
			console.log(
				`Error while migrating missing email templates for ${EmailTemplateNameEnum.EMAIL_RESET}`,
				error
			);
		}
		try {
			...
			console.log(
				`Error while migrating missing email templates for ${EmailTemplateNameEnum.ORGANIZATION_TEAM_JOIN_REQUEST}`,
				error
			);
		}
	}
