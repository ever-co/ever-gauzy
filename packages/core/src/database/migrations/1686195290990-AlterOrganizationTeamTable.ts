...
export class AlterOrganizationTeamTable1686195290990
	implements MigrationInterface
{
	...
	public async postgresUpQueryRunner(queryRunner: QueryRunner): Promise<any> {
		await queryRunner.query(
			`ALTER TABLE "organization_team" ADD "color" character varying`
		);
		await queryRunner.query(
			`ALTER TABLE "organization_team" ADD "emoji" character varying`
		);
		await queryRunner.query(
			`ALTER TABLE "organization_team" ADD "teamSize" character varying`
		);
	}
	...
	public async postgresDownQueryRunner(
		...
		await queryRunner.query(
			`ALTER TABLE "organization_team" DROP COLUMN "teamSize"`
		);
		await queryRunner.query(
			`ALTER TABLE "organization_team" DROP COLUMN "emoji"`
		);
		await queryRunner.query(
			`ALTER TABLE "organization_team" DROP COLUMN "color"`
		);
	}
	...
	public async sqliteUpQueryRunner(queryRunner: QueryRunner): Promise<any> {
		...
		await queryRunner.query(
			`CREATE INDEX "IDX_51e91be110fa0b8e70066f5727" ON "organization_team" ("imageId") `
		);
		await queryRunner.query(
