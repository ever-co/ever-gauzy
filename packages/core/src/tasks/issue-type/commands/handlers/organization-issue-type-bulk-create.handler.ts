...
export class OrganizationIssueTypeBulkCreateHandler implements ICommandHandler<OrganizationIssueTypeBulkCreateCommand> {
	constructor(private readonly issueTypeService: IssueTypeService) {}

	public async execute(command: OrganizationIssueTypeBulkCreateCommand): Promise<IIssueType[]> {
		const { input } = command;
