...
export class GetOrganizationTeamStatisticHandler implements IQueryHandler<GetOrganizationTeamStatisticQuery> {
	...
	async execute(input: GetOrganizationTeamStatisticQuery): Promise<IOrganizationTeam> {
		try {
			...
			// A deliberate HTTP answer from below — the sensitive-relation check answers 403 here — must
			// reach the caller as itself. Flattening everything into a 400 hides which rule refused, and
			// told a caller probing relations that the request was malformed rather than forbidden.
