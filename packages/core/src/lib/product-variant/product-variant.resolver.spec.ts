...
describe('ProductVariantResolver — the guard stack is the controller’s', () => {
	...
	it('runs the count route under the guard chain the resolver states', () => {
		...
		expect([...guardsOfRoute(ProductVariantController, 'getCount'), FeatureFlagGuard].sort()).toEqual(
			[...stated].sort()
		);
...
function graphqlContext(field: string): ExecutionContext {
	return {
		getHandler: () => (ProductVariantResolver.prototype as never)[field],
		getClass: () => ProductVariantResolver,
		getType: () => 'graphql',
		getArgByIndex: () => ({ fieldName: field })
	} as unknown as ExecutionContext;
}

describe('ProductVariantResolver — a capability that is switched off is not served', () => {
	it('declares the capability the commerce catalogue declares for this surface, on the class', () => {
		// One statement, read by the guard with `getAllAndOverride` over the handler and then the class,
		// so every field is behind it.
		expect(Reflect.getMetadata(FEATURE_METADATA, ProductVariantResolver)).toBe(FEATURE_GRAPHQL);
		expect(Reflect.getMetadata('__guards__', ProductVariantResolver)).toContain(FeatureFlagGuard);
	});

	it('refuses a field whose capability is switched off, and names the field it refused', async () => {
		const { guard, featureService } = gate(false);

		const refusal = await guard.canActivate(graphqlContext('productVariants')).catch((thrown) => thrown);

		// The code the guard resolved is the one this resolver declared, not a second copy of it.
		expect(featureService.isFeatureEnabled).toHaveBeenCalledWith(FEATURE_GRAPHQL);
		expect(refusal).toBeInstanceOf(NotFoundException);
		// A disabled capability answers the way a missing one does, and says which field was refused.
		expect((refusal as Error).message).toContain('productVariants');
	...
	it('serves the field once the capability is switched on', async () => {
		...
		await expect(guard.canActivate(graphqlContext('productVariants'))).resolves.toBe(true);
	});
});
