...
describe('EquipmentSharingPolicyResolver — the SDL declares the capabilities the REST routes serve', () => {
	...
	it('offers the soft-delete switch its own route offers, and no filter it could not honour', () => {
		...
		// same read, so a REST caller can ask for withdrawn policies — and the connection has to be able
		...
		expect(fieldArgs('Query', 'equipmentSharingPolicies')).toContain('withDeleted');
...
describe('EquipmentSharingPolicyResolver — the connection contract', () => {
	...
	it('passes the soft-delete switch through to the read rather than declaring it and dropping it', async () => {
		...
		// A stated switch is the route's own option travelling the same way: a withdrawn policy is
		...
		await resolver.equipmentSharingPolicies(undefined, undefined, undefined, undefined, undefined, undefined, undefined, undefined, undefined, true);

		expect(equipmentSharingPolicyService.findAll).toHaveBeenCalledWith({ withDeleted: true });
	});
