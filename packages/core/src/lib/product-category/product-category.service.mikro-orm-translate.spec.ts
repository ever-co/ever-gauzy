 ...
 * `product.service.mikro-orm-translate.spec.ts`), so the reads are collected there and asserted below. Nothing is
 * stood in for: the translations a MikroORM create cascades get their key from `BaseEntity.id`'s `onCreate` on
 * SQLite, as they get it from the table's default on PostgreSQL.
 */
