...
describe('UserCreateHandler (GHSA-jh6m-9fxr-rx3c)', () => {
	...
	it('still validates the roles being assigned, from both the flat id and the relation', async () => {
		...
		expect(assertCanAssignRoles).toHaveBeenCalledWith(['flat-role', 'relation-role']);
	});
});
