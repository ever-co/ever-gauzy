...
export class CreateUserNotificationTable1738217979804 implements MigrationInterface {
	...
	public async postgresUpQueryRunner(queryRunner: QueryRunner): Promise<any> {
		await queryRunner.query(
			`CREATE TABLE "user_notification" ("deletedAt" TIMESTAMP, "createdAt" TIMESTAMP NOT NULL DEFAULT now(), "updatedAt" TIMESTAMP NOT NULL DEFAULT now(), "id" uuid NOT NULL DEFAULT gen_random_uuid(), "isActive" boolean DEFAULT true, "isArchived" boolean DEFAULT false, "archivedAt" TIMESTAMP, "tenantId" uuid, "organizationId" uuid, "entity" character varying NOT NULL, "entityId" character varying NOT NULL, "title" character varying, "message" character varying, "type" integer, "isRead" boolean NOT NULL DEFAULT false, "readAt" TIMESTAMP, "onHoldUntil" TIMESTAMP, "sentById" uuid, "receiverId" uuid, CONSTRAINT "PK_8840aac86dec5f669c541ce67d4" PRIMARY KEY ("id"))`
		);
		...
		await queryRunner.query(
			`CREATE INDEX "IDX_758686e0a822303c4791999a77" ON "user_notification" ("organizationId") `
		);
		...
		await queryRunner.query(
			`ALTER TABLE "user_notification" ADD CONSTRAINT "FK_6b218dc20d7822e0e4d85d81a2a" FOREIGN KEY ("tenantId") REFERENCES "tenant"("id") ON DELETE CASCADE ON UPDATE NO ACTION`
		);
		await queryRunner.query(
			`ALTER TABLE "user_notification" ADD CONSTRAINT "FK_758686e0a822303c4791999a778" FOREIGN KEY ("organizationId") REFERENCES "organization"("id") ON DELETE CASCADE ON UPDATE CASCADE`
		);
		await queryRunner.query(
			`ALTER TABLE "user_notification" ADD CONSTRAINT "FK_08d1dae7d71c5eb56d06edf10f0" FOREIGN KEY ("sentById") REFERENCES "user"("id") ON DELETE CASCADE ON UPDATE NO ACTION`
		);
		await queryRunner.query(
			`ALTER TABLE "user_notification" ADD CONSTRAINT "FK_ff2d84b36dcf89af3e9b13f643f" FOREIGN KEY ("receiverId") REFERENCES "user"("id") ON DELETE CASCADE ON UPDATE NO ACTION`
		);
	}
	...
	public async sqliteUpQueryRunner(queryRunner: QueryRunner): Promise<any> {
		...
		await queryRunner.query(
			`CREATE TABLE "temporary_user_notification" ("deletedAt" datetime, "createdAt" datetime NOT NULL DEFAULT (datetime('now')), "updatedAt" datetime NOT NULL DEFAULT (datetime('now')), "id" varchar PRIMARY KEY NOT NULL, "isActive" boolean DEFAULT (1), "isArchived" boolean DEFAULT (0), "archivedAt" datetime, "tenantId" varchar, "organizationId" varchar, "entity" varchar NOT NULL, "entityId" varchar NOT NULL, "title" varchar, "message" varchar, "type" integer, "isRead" boolean NOT NULL DEFAULT (0), "readAt" datetime, "onHoldUntil" datetime, "sentById" varchar, "receiverId" varchar, CONSTRAINT "FK_6b218dc20d7822e0e4d85d81a2a" FOREIGN KEY ("tenantId") REFERENCES "tenant" ("id") ON DELETE CASCADE ON UPDATE NO ACTION, CONSTRAINT "FK_758686e0a822303c4791999a778" FOREIGN KEY ("organizationId") REFERENCES "organization" ("id") ON DELETE CASCADE ON UPDATE CASCADE, CONSTRAINT "FK_08d1dae7d71c5eb56d06edf10f0" FOREIGN KEY ("sentById") REFERENCES "user" ("id") ON DELETE CASCADE ON UPDATE NO ACTION, CONSTRAINT "FK_ff2d84b36dcf89af3e9b13f643f" FOREIGN KEY ("receiverId") REFERENCES "user" ("id") ON DELETE CASCADE ON UPDATE NO ACTION)`
		);
		await queryRunner.query(
			`INSERT INTO "temporary_user_notification"("deletedAt", "createdAt", "updatedAt", "id", "isActive", "isArchived", "archivedAt", "tenantId", "organizationId", "entity", "entityId", "title", "message", "type", "isRead", "readAt", "onHoldUntil", "sentById", "receiverId") SELECT "deletedAt", "createdAt", "updatedAt", "id", "isActive", "isArchived", "archivedAt", "tenantId", "organizationId", "entity", "entityId", "title", "message", "type", "isRead", "readAt", "onHoldUntil", "sentById", "receiverId" FROM "user_notification"`
		);
		await queryRunner.query(`DROP TABLE "user_notification"`);
	...
	public async sqliteDownQueryRunner(queryRunner: QueryRunner): Promise<any> {
		...
		await queryRunner.query(
			`INSERT INTO "user_notification"("deletedAt", "createdAt", "updatedAt", "id", "isActive", "isArchived", "archivedAt", "tenantId", "organizationId", "entity", "entityId", "title", "message", "type", "isRead", "readAt", "onHoldUntil", "sentById", "receiverId") SELECT "deletedAt", "createdAt", "updatedAt", "id", "isActive", "isArchived", "archivedAt", "tenantId", "organizationId", "entity", "entityId", "title", "message", "type", "isRead", "readAt", "onHoldUntil", "sentById", "receiverId" FROM "temporary_user_notification"`
		);
	...
	public async mysqlUpQueryRunner(queryRunner: QueryRunner): Promise<any> {
		await queryRunner.query(
			`CREATE TABLE \`user_notification\` (\`deletedAt\` datetime(6) NULL, \`createdAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updatedAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`id\` varchar(36) NOT NULL, \`isActive\` tinyint NULL DEFAULT 1, \`isArchived\` tinyint NULL DEFAULT 0, \`archivedAt\` datetime NULL, \`tenantId\` varchar(255) NULL, \`organizationId\` varchar(255) NULL, \`entity\` varchar(255) NOT NULL, \`entityId\` varchar(255) NOT NULL, \`title\` varchar(255) NULL, \`message\` varchar(255) NULL, \`type\` int NULL, \`isRead\` tinyint NOT NULL DEFAULT 0, \`readAt\` datetime NULL, \`onHoldUntil\` datetime NULL, \`sentById\` varchar(255) NULL, \`receiverId\` varchar(255) NULL, INDEX \`IDX_6c5f59af45edf24ebe1fb9290a\` (\`isActive\`), INDEX \`IDX_1162ba6e6e4a847017c1edc88b\` (\`isArchived\`), INDEX \`IDX_6b218dc20d7822e0e4d85d81a2\` (\`tenantId\`), INDEX \`IDX_758686e0a822303c4791999a77\` (\`organizationId\`), INDEX \`IDX_6af19d4e214ca45b5fb57d7fc2\` (\`entity\`), INDEX \`IDX_a6de2f2e523b7428765d9fabbe\` (\`entityId\`), INDEX \`IDX_fc3e6d279904252599ae41f5bf\` (\`type\`), INDEX \`IDX_08d1dae7d71c5eb56d06edf10f\` (\`sentById\`), INDEX \`IDX_ff2d84b36dcf89af3e9b13f643\` (\`receiverId\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`
		);
		await queryRunner.query(
			`ALTER TABLE \`user_notification\` ADD CONSTRAINT \`FK_6b218dc20d7822e0e4d85d81a2a\` FOREIGN KEY (\`tenantId\`) REFERENCES \`tenant\`(\`id\`) ON DELETE CASCADE ON UPDATE NO ACTION`
		);
	...
	public async mysqlDownQueryRunner(queryRunner: QueryRunner): Promise<any> {
		await queryRunner.query(
			`ALTER TABLE \`user_notification\` DROP FOREIGN KEY \`FK_ff2d84b36dcf89af3e9b13f643f\``
		);
		await queryRunner.query(
			`ALTER TABLE \`user_notification\` DROP FOREIGN KEY \`FK_08d1dae7d71c5eb56d06edf10f0\``
		);
		await queryRunner.query(
			`ALTER TABLE \`user_notification\` DROP FOREIGN KEY \`FK_758686e0a822303c4791999a778\``
		);
		await queryRunner.query(
			`ALTER TABLE \`user_notification\` DROP FOREIGN KEY \`FK_6b218dc20d7822e0e4d85d81a2a\``
		);
