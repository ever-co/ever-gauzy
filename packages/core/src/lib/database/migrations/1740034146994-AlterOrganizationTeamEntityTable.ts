...
export class AlterOrganizationTeamEntityTable1740034146994 implements MigrationInterface {
	...
	public async sqliteUpQueryRunner(queryRunner: QueryRunner): Promise<any> {
		...
		await queryRunner.query(`
			CREATE TABLE "temporary_organization_team" (
				...
				"deletedAt" datetime,
				"shareProfileView" boolean DEFAULT (1),
				"requirePlanToTrack" boolean NOT NULL DEFAULT (0),
				"archivedAt" datetime,
				CONSTRAINT "FK_51e91be110fa0b8e70066f5727f" FOREIGN KEY ("imageId") REFERENCES "image_asset" ("id") ON DELETE SET NULL ON UPDATE NO ACTION,
				CONSTRAINT "FK_eef1c19a0cb5321223cfe3286c4" FOREIGN KEY ("organizationId") REFERENCES "organization" ("id") ON DELETE CASCADE ON UPDATE CASCADE,
				CONSTRAINT "FK_176f5ed3c4534f3110d423d5690" FOREIGN KEY ("tenantId") REFERENCES "tenant" ("id") ON DELETE CASCADE ON UPDATE NO ACTION
			);
		`);

		// Step 3: Insert data from the old "organization_team" to the new temporary table.
		console.log(`Insert data from the old "organization_team" to the new temporary table`);
		await queryRunner.query(`
			INSERT INTO "temporary_organization_team" (
				"id",
				"createdAt",
				"updatedAt",
				"tenantId",
		...
		// Step 4: Drop the old "organization_team" table and rename the temporary table.
		console.log(`Step 4: Drop the old "organization_team" table and rename the temporary table.`);
		...
		// Step 5: Create indexes on the new "organization_team" table.
		console.log(`Create indexes on the new "organization_team" table`);
	...
	public async sqliteDownQueryRunner(queryRunner: QueryRunner): Promise<any> {
		...
		await queryRunner.query(`
			CREATE TABLE "organization_team" (
				...
				"archivedAt" datetime,
				CONSTRAINT "FK_51e91be110fa0b8e70066f5727f" FOREIGN KEY ("imageId") REFERENCES "image_asset" ("id") ON DELETE SET NULL ON UPDATE NO ACTION,
				CONSTRAINT "FK_da625f694eb1e23e585f3010082" FOREIGN KEY ("createdById") REFERENCES "user" ("id") ON DELETE SET NULL ON UPDATE NO ACTION,
				CONSTRAINT "FK_eef1c19a0cb5321223cfe3286c4" FOREIGN KEY ("organizationId") REFERENCES "organization" ("id") ON DELETE CASCADE ON UPDATE CASCADE,
				CONSTRAINT "FK_176f5ed3c4534f3110d423d5690" FOREIGN KEY ("tenantId") REFERENCES "tenant" ("id") ON DELETE CASCADE ON UPDATE NO ACTION
			)
		`);
		...
		await queryRunner.query(`
			...
			SELECT
				...
				"requirePlanToTrack",
				"archivedAt"
			FROM "temporary_organization_team";
		`);

		// Step 5: Drop the old "organization_team" table and rename the temporary table.
		console.log('Step 5: Dropping the old "temporary_organization_team" table...');
		await queryRunner.query(`DROP TABLE "temporary_organization_team"`);

		// Step 6: Create final indexes on the new "organization_team" table.
		console.log('Step 6: Creating indexes on the new "organization_team" table...');
		await queryRunner.query(
			`CREATE INDEX "IDX_e22ab0f1236b1a07785b641727" ON "organization_team" ("profile_link") `
		);
		await queryRunner.query(`CREATE INDEX "IDX_176f5ed3c4534f3110d423d569" ON "organization_team" ("tenantId") `);
