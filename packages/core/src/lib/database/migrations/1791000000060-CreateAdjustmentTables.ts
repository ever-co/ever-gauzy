...
export class CreateAdjustmentTables1791000000060 implements MigrationInterface {
	...
	public async postgresUpQueryRunner(queryRunner: QueryRunner): Promise<any> {
		...
		await queryRunner.query(
			`CREATE UNIQUE INDEX "UQ_adjustment_reason_org_code" ON "adjustment_reason" ("organizationId", "code") WHERE "deletedAt" IS NULL`
		);
		await queryRunner.query(
	...
	public async mysqlUpQueryRunner(queryRunner: QueryRunner): Promise<any> {
		...
		await queryRunner.query(
			`CREATE TABLE \`adjustment_reason\` (\`deletedAt\` datetime(6) NULL, \`createdAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updatedAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`createdByUserId\` varchar(36) NULL, \`updatedByUserId\` varchar(36) NULL, \`deletedByUserId\` varchar(36) NULL, \`id\` varchar(36) NOT NULL, \`isActive\` tinyint NULL DEFAULT 1, \`isArchived\` tinyint NULL DEFAULT 0, \`archivedAt\` datetime NULL, \`tenantId\` varchar(36) NULL, \`organizationId\` varchar(36) NULL, \`code\` varchar(64) NOT NULL, \`label\` varchar(255) NOT NULL, \`description\` text NULL, \`appliesTo\` varchar(32) NOT NULL DEFAULT 'MANUAL', \`requiresApproval\` tinyint NOT NULL DEFAULT 0, \`isSystem\` tinyint NOT NULL DEFAULT 0, \`sortOrder\` int NOT NULL DEFAULT 0, \`metadata\` json NULL, INDEX \`IDX_adjustment_reason_created_by_user\` (\`createdByUserId\`), INDEX \`IDX_adjustment_reason_updated_by_user\` (\`updatedByUserId\`), INDEX \`IDX_adjustment_reason_deleted_by_user\` (\`deletedByUserId\`), INDEX \`IDX_adjustment_reason_is_active\` (\`isActive\`), INDEX \`IDX_adjustment_reason_is_archived\` (\`isArchived\`), INDEX \`IDX_adjustment_reason_tenant\` (\`tenantId\`), INDEX \`IDX_adjustment_reason_organization\` (\`organizationId\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`
		);
		...
		await queryRunner.query(
			`CREATE UNIQUE INDEX \`UQ_adjustment_reason_org_code\` ON \`adjustment_reason\` (\`organizationId\`, \`code\`, \`deletedAt\`)`
		);
		await queryRunner.query(
