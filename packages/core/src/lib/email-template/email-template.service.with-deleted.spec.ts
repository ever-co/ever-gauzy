...
/**
 * SD-5 — `emailTemplates(withDeleted: true)` and the REST list route beside it answered exactly the
 ...
 * soft-delete predicate must never lift the tenant scope (GHSA-44pv-34gx-q9p4) with it.
 */

const TENANT_A = '5e000000-0000-4000-8000-00000000000a';
const TENANT_B = '5e000000-0000-4000-8000-00000000000b';
const ORGANIZATION_A = '5e000000-0000-4000-8000-0000000000a1';
const ORGANIZATION_B = '5e000000-0000-4000-8000-0000000000b1';
const RETIRED_AT = new Date('2026-01-01T00:00:00.000Z');

/** The rows every case reads, identified by `hbs` so a leak names itself rather than moving a count. */
const SEED = [
	{ id: '5e000000-0000-4000-8000-000000000001', hbs: 'tenant-live', tenantId: TENANT_A, organizationId: ORGANIZATION_A, deletedAt: null },
	{ id: '5e000000-0000-4000-8000-000000000002', hbs: 'tenant-retired', tenantId: TENANT_A, organizationId: ORGANIZATION_A, deletedAt: RETIRED_AT },
	{ id: '5e000000-0000-4000-8000-000000000003', hbs: 'other-tenant-retired', tenantId: TENANT_B, organizationId: ORGANIZATION_B, deletedAt: RETIRED_AT },
	{ id: '5e000000-0000-4000-8000-000000000004', hbs: 'global-live', tenantId: null, organizationId: null, deletedAt: null },
	{ id: '5e000000-0000-4000-8000-000000000005', hbs: 'global-retired', tenantId: null, organizationId: null, deletedAt: RETIRED_AT }
].map((row) => ({ ...row, name: 'welcome/html', languageCode: 'en' }));
 ...
 */
@SoftDeletable(() => EmailTemplateWithDeletedRow, 'deletedAt', () => new Date())
@Entity({ tableName: 'email_template' })
class EmailTemplateWithDeletedRow {
...
async function typeOrmHarness(): Promise<IHarness> {
	...
	const templates: Repository<any> = dataSource.getRepository('EmailTemplate');
...
async function mikroOrmHarness(): Promise<IHarness> {
	const orm = await MikroORM.init({
		...
		dbName: ':memory:',
		entities: [EmailTemplateWithDeletedRow],
	...
	await em.persistAndFlush(SEED.map((row) => em.create(EmailTemplateWithDeletedRow, row)));
	...
	return {
		...
		service: () =>
			new EmailTemplateService({} as any, orm.em.fork().getRepository(EmailTemplateWithDeletedRow) as any),
...
describe.each([
...
])('EmailTemplateService.findAll honours withDeleted (%s)', (ormType, createHarness) => {
	...
	/** The rows one read answered, sorted. */
	const read = async (options: Record<string, unknown>): Promise<string[]> => {
		...
		return items.map((item: any) => item.hbs).sort();
	};
