...
describe('EquipmentResolver — the SDL declares the capabilities the REST routes serve', () => {
	...
	it('offers the soft-delete switch its own route offers, and no filter it could not honour', () => {
		...
		// same read, so a REST caller can ask for withdrawn assets — and the connection has to be able
		...
		expect(fieldArgs('Query', 'equipments')).toContain('withDeleted');
...
describe('EquipmentResolver — the connection contract', () => {
	...
	it('passes the soft-delete switch through to the read rather than declaring it and dropping it', async () => {
		...
		// A stated switch is the route's own option travelling the same way: a withdrawn asset is
		...
		await resolver.equipments(undefined, undefined, undefined, undefined, undefined, undefined, undefined, undefined, undefined, true);

		expect(equipmentService.findAll).toHaveBeenCalledWith({ withDeleted: true });
	});
