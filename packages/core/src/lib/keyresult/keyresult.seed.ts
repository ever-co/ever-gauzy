...
export const createDefaultKeyResults = async (
	...
	if (goals && goals.length > 0) {
		goals.forEach((goal) => {
			const keyResultsOfGoal = DEFAULT_KEY_RESULTS.find((goalData) => goalData.name === goal.name);
			keyResultsOfGoal.keyResults.forEach((keyResultData) => {
				...
				if (keyResult.deadline !== KeyResultDeadlineEnum.NO_CUSTOM_DEADLINE) {
					...
					if (keyResult.deadline === KeyResultDeadlineEnum.HARD_AND_SOFT_DEADLINE) {
...
export const updateDefaultKeyResultProgress = async (dataSource: DataSource): Promise<KeyResult[]> => {
	...
	await Promise.all(
		keyResults.map(async (keyResult) => {
			const sortedUpdates: KeyResultUpdate[] = [...keyResult.updates].sort((a, b) => {
...
const insertDefaultKeyResults = async (dataSource: DataSource, defaultKeyResults: KeyResult[]) => {
	await dataSource.createQueryBuilder().insert().into(KeyResult).values(defaultKeyResults).execute();
};
	...
	for (const tenant of tenants) {
		...
		for await (const organization of tenantOrgs) {
			...
			for (const goal of goals) {
				...
				keyResult.deadline = faker.helpers.arrayElement(Object.keys(KeyResultDeadlineEnum));
				if (keyResult.deadline !== KeyResultDeadlineEnum.NO_CUSTOM_DEADLINE) {
				...
				keyResult.type = faker.helpers.arrayElement(Object.keys(KeyResultTypeEnum));
				...
				if (keyResult.type === KeyResultTypeEnum.TRUE_OR_FALSE) {
					...
					keyResult.initialValue = faker.number.int(keyResult.targetValue);
				}

				keyResult.unit = faker.helpers.arrayElement(['signups', 'publications', 'interviews', 'people', '%']);
