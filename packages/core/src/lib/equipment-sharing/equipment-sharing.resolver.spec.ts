...
describe('EquipmentSharingResolver — the SDL declares the capabilities the REST routes serve', () => {
	...
	it('offers the soft-delete switch its own route offers, and no filter it could not honour', () => {
		...
		// same read, so a REST caller can ask for withdrawn periods — and the connection has to be able
		...
		expect(fieldArgs('Query', 'equipmentSharings')).toContain('withDeleted');
...
describe('EquipmentSharingResolver — the connection contract', () => {
	...
	it('passes the soft-delete switch through to the read rather than declaring it and dropping it', async () => {
		...
		// A stated switch is the route's own option travelling the same way: a withdrawn period is
		...
		await resolver.equipmentSharings(undefined, undefined, undefined, undefined, undefined, undefined, undefined, undefined, undefined, true);

		expect(equipmentSharingService.findAll).toHaveBeenCalledWith({ withDeleted: true });
	});
