 ...
 * is not meaningful for this resource anyway: a currency is installation-wide reference data and its table
 * carries no tenancy column, so there is no tenant whose rows could give a different answer. The surface
 * is withheld, when an operator wants it withheld, by the gate on the resolvers that read tenant data and
 * by the route-level gate on the REST side — not by a per-tenant check on a row that belongs to no tenant.
