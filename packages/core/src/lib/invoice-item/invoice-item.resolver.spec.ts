...
describe('InvoiceItemResolver — the SDL declares the capabilities the REST routes serve', () => {
	...
	it('offers the soft-delete switch its own route offers, and no filter it could not honour', () => {
		...
		// same read, so a REST caller can ask for withdrawn lines — and the connection has to be able
		...
		expect(fieldArgs('Query', 'invoiceItems')).toContain('withDeleted');
...
describe('InvoiceItemResolver — the connection contract', () => {
	...
	it('passes the soft-delete switch through to the read rather than declaring it and dropping it', async () => {
		...
		// A stated switch is the route's own option travelling the same way: a withdrawn line is
		...
		await resolver.invoiceItems(undefined, undefined, undefined, undefined, undefined, undefined, undefined, undefined, undefined, true);

		expect(invoiceItemService.findAll).toHaveBeenCalledWith({ relations: [], withDeleted: true });
	});
