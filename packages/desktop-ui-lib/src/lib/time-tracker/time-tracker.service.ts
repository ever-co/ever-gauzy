...
export class TimeTrackerService {
	...
	async getProjects(values) {
		...
		if (!projects$) {
			projects$ = this.http
				.get(`${values.apiHost}/api/organization-projects/employee/${values.employeeId}`, {
					params: toParams(params)
				})
				.pipe(
					map((response: any) => response),
	...
	async getClient(values) {
		...
		if (!clients$) {
			clients$ = this.http
				.get(`${values.apiHost}/api/organization-contact/employee/${values.employeeId}`, {
					params
				})
				.pipe(
					map((response: any) => response),
	...
	toggleApiStart(values) {
		...
		return firstValueFrom(this.http.post(`${values.apiHost}/api/timesheet/timer/start`, { ...body }));
	}
	...
	toggleApiStop(values) {
		...
		return firstValueFrom(this.http.post(`${values.apiHost}/api/timesheet/timer/stop`, { ...body }));
	}
	...
	pushToTimeSlot(values) {
		...
		return firstValueFrom(
			this.http.post(`${values.apiHost}/api/timesheet/time-slot`, params).pipe(
				catchError((error) => {
					error.error = {
						...error.error,
	...
	uploadImages(values, img: any) {
		...
		return firstValueFrom(
			this.http.post(`${values.apiHost}/api/timesheet/screenshot`, formData).pipe(
				catchError((error) => {
					error.error = {
						...error.error,
	...
	saveNewTask(values, payload) {
		return firstValueFrom(
			this.http.post(`${values.apiHost}/api/tasks`, payload).pipe(
	...
	createNewContact(input: IOrganizationContactCreateInput, values): Promise<IOrganizationContact> {
		return firstValueFrom(
			this.http.post<IOrganizationContact>(`${values.apiHost}/api/organization-contact`, input).pipe(
