...
export class SettingsComponent implements OnInit, AfterViewInit {
	...
	serverOptions = this.isDesktopTimer
		...
		: [this.serverTypes.integrated, this.serverTypes.localNetwork, this.serverTypes.live];
	...
	public async checkHostConnectivity(): Promise<any> {
		try {
			...
			this._isCheckHost$.next({
				...
				isHidden: false,
				isLoading: false,
				message: isOkAnyway ? `Connection to Server ${this.config.serverUrl} Succeeds` : error.message
			});
