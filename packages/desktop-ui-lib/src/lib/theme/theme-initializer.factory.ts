...
export function ThemeInitializerFactory(themeService: NbThemeService, electronService: ElectronService) {
	return async () => {
		const theme = await electronService.ipcRenderer.invoke('PREFERRED_THEME');
