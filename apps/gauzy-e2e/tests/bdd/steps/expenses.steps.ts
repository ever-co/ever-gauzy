...
// Converted 1:1 from the plain ExpensesTest.spec.ts: the single test() -> one Scenario, each
...
// default user` Background step is defined once in common.steps.ts. The faker-generated `name` used
// across steps is hoisted to module scope and initialised at the start of the first step.

let name = ' ';

When('I add a new expense', async () => {
...
When('I duplicate the expense', async () => {
	await expensesPage.waitMessageToHide();
	await expensesPage.selectTableRow(0);
...
When('I delete the expense', async () => {
	await expensesPage.waitMessageToHide();
	await expensesPage.selectTableRow(0);
