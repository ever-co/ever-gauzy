...
// Converted 1:1 from the plain MyTasksTrackedInTimesheetsTest.spec.ts: the single test() -> one
// Scenario, each test.step() -> one When step whose body is the verbatim .po/CustomCommands call
// sequence (verification folded in), so runtime behaviour is identical to the already-CI-tested
// spec. The `Given I am logged in as the default user` Background step is defined once in
// common.steps.ts. Cross-step faker state is hoisted to module scope and initialised at the top of
// the first step (one scenario per feature => safe).
...
When('I add a new tag as admin for the timesheet flow', async () => {
...
When('I add a new employee for the timesheet flow', async () => {
...
When('I add a new project assigned to the timesheet employee', async () => {
	await CustomCommands.logout(dashboardPage, logoutPage, loginPage);
	await CustomCommands.clearCookies();
	await CustomCommands.login(loginPage, LoginPageData, dashboardPage);
...
When('I log out as admin from the timesheet flow', async () => {
	await CustomCommands.logout(dashboardPage, logoutPage, loginPage);
	await CustomCommands.clearCookies();
});

When('I log in as the timesheet employee', async () => {
	await CustomCommands.loginAsEmployee(loginPage, dashboardPage, employeeEmail, password);
});

When('I create a task from My Tasks as the employee', async () => {
...
When('I record time against the new My Tasks task', async () => {
...
When('I stop the timer and view the My Tasks timesheet', async () => {
