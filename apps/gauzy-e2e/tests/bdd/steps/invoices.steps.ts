...
// Converted 1:1 from the plain InvoicesTest.spec.ts: the single test() -> one Scenario, each
...
When('I add a new invoice', async () => {
...
When('I search for an invoice', async () => {
...
When('I send the invoice by email', async () => {
	await invoicesPage.selectTableRow(0, fullName);
	await invoicesPage.moreButtonVisible();
	await invoicesPage.clickMoreButton();
	await invoicesPage.actionButtonVisible();
...
When('I set the invoice status', async () => {
	await invoicesPage.waitMessageToHide();
	await invoicesPage.selectTableRow(0, fullName);
