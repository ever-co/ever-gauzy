...
// Converted 1:1 from the plain DangerZoneTest.spec.ts: the single test() -> one Scenario, its single
// test.step() -> one When step whose body is the verbatim .po call sequence (including the goto/hash
// settle + card-header wait hardening), so runtime behaviour is identical to the already-CI-tested spec.
// The `Given I am logged in as the default user` Background step is defined once in common.steps.ts.
//
// NOTE: the plain spec was `test.describe.skip(...)` — SKIPPED because the danger-zone feature is hidden
// in the build the e2e suite runs against, not a test defect. danger-zone.component.html gates the entire
// card body (Delete account / Delete all data buttons + dialogs) behind `@if (!environment.DEMO)`, and
// environment.DEMO is hardcoded `true` in BOTH
...
When('I verify the danger zone', async () => {
