...
/**
 * Converted 1:1 from the plain OrganizationPublicPageTest.spec.ts: the single test() -> one Scenario,
 * each test.step() -> one When step whose body is the verbatim .po/CustomCommands call sequence, so
 * runtime behaviour is identical to the already-CI-tested spec. The `Given I am logged in as the
 * default user` Background step is defined once in common.steps.ts. Faker-generated values shared across
 * steps are declared at module scope (one scenario per feature, so no cross-scenario bleed) and the
 * dashboard verification that followed login in the plain spec is folded into the first step body.
 *
...
When('I add a new organization', async () => {
...
When('I add a new employee to the organization', async () => {
...
When('I add a new project to the organization', async () => {
	await selectOrganization(organizationName);
	try {
...
When('I add a new client to the organization', async () => {
	await selectOrganization(organizationName);
	try {
...
When('I add a public profile link to the organization', async () => {
...
When('I edit the organization public page', async () => {
	try {
...
When('I verify the organization public page data', async () => {
	try {
