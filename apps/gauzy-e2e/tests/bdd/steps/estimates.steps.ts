...
// Converted 1:1 from the plain EstimatesTest.spec.ts: the single test() -> one Scenario, each
...
// live at module scope and are initialised at the start of the first step. The `Given I am logged in as
...
When('I add a new estimate', async () => {
...
When('I search for an estimate', async () => {
...
When('I edit the estimate', async () => {
...
When('I duplicate the estimate', async () => {
	await estimatesPage.waitMessageToHide();
	await estimatesPage.selectTableRow(fullName);
	await estimatesPage.selectTableRow(fullName);
...
When('I send the estimate', async () => {
	await estimatesPage.selectTableRow(fullName);
	await estimatesPage.moreButtonVisible();
	await estimatesPage.clickMoreButton();
	await estimatesPage.actionButtonVisible();
...
When('I view the estimate', async () => {
	await estimatesPage.selectTableRow(fullName);
...
When('I send the estimate by email', async () => {
	await estimatesPage.selectTableRow(fullName);
	await estimatesPage.moreButtonVisible();
	await estimatesPage.clickMoreButton();
	await estimatesPage.actionButtonVisible();
...
When('I convert the estimate to an invoice', async () => {
	await estimatesPage.selectTableRow(fullName);
...
When('I delete the estimate', async () => {
	await estimatesPage.addButtonVisible();
	await estimatesPage.clickAddButton();
	await estimatesPage.tagsDropdownVisible();
	await estimatesPage.clickTagsDropdown();
