...
// Converted 1:1 from the plain EditEmployeeTest.spec.ts: the single test() -> one Scenario, each
// test.step() -> one When step whose body is the verbatim .po call sequence, so runtime behaviour is
// identical to the already-CI-tested spec. The faker-generated fixtures shared across steps are
// declared at module scope and initialised at the top of the first step, and the pre-step
// CustomCommands setup (addEmployee/addProject/addTag/addContact) that ran after login but before the
// first test.step() is folded into the first step's body. The `Given I am logged in as the default
...
When('I edit the employee account data', async () => {
...
When('I edit the employee network data', async () => {
...
When('I edit the employee employment data', async () => {
...
When('I edit the employee hiring data', async () => {
...
When('I edit the employee location data', async () => {
...
When('I edit the employee rates data', async () => {
...
When('I edit the employee projects data', async () => {
...
When('I edit the employee contacts data', async () => {
	await editEmployeePage.addProjectOrContactButtonVisible();
	await editEmployeePage.clickAddProjectOrContactButton();
	await editEmployeePage.projectOrContactDropdownVisible();
	await editEmployeePage.clickProjectOrContactDropdown();
