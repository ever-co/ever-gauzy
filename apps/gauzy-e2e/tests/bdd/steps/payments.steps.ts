...
// Converted 1:1 from the plain PaymentsTest.spec.ts: the single test() -> one Scenario, each
// test.step() -> one When step whose body is the verbatim statement sequence (verification folded in),
...
When('I add a new payment', async () => {
...
When('I delete the payment', async () => {
	await paymentsPage.tableRowVisible();
	await paymentsPage.selectTableRow(0);
