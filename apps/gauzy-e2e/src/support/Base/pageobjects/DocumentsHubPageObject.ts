...
export const DocumentsHubPage = {
	...
	// The menu items are NbMenuItem MODELS, not template elements, so their icons are `[config]`-bound:
	// no `icon` attribute reaches the DOM and the icon-name convention above cannot be used here.
	// Nebular renders each item as `<a [attr.title]>…<span class="menu-title">`, and neither carries
	// anything language-independent — a `data-testid` would have to be threaded through Nebular's own
	// menu rendering, so matching the label is the only option short of replacing the context menu.
	// Playwright's `:has-text()` is a case-insensitive SUBSTRING match, which absorbs the "New page"
	// vs "New Page" difference between the docs-ui and ui-core catalogues, and "New page" cannot match
	// the sibling "New folder".
	//
	// This depends on the UI still being in English. That holds by ORDERING, not by luck:
	// `change-language.feature` runs in a different shard (own machine, own DB, own browser), and the
	// only spec that changes THIS session's language is `edit-profile.feature`, which sorts after
	// `documents-hub.feature` within the shard. If specs are ever reordered or re-sharded, this is the
	// first selector to break — the fix then is a real hook on the menu, not a different string.
