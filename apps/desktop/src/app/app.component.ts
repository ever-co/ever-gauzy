...
export class AppComponent implements OnInit {
	constructor(private router: Router, private electronService: ElectronService, private appService: AppService) {
		this.electronService.ipcRenderer.on('collect_data', (event, arg) => {
			appService.collectEvents(arg.tpURL, arg.tp, arg.start, arg.end).then((res) => {
		...
		this.electronService.ipcRenderer.on('collect_afk', (event, arg) => {
			appService.collectAfk(arg.tpURL, arg.tp, arg.start, arg.end).then((res) => {
				event.sender.send('data_push_activity', {
					timerId: arg.timerId,
					windowEvent: res,
					type: 'AFK'
		...
		this.electronService.ipcRenderer.on('collect_chrome_activities', (event, arg) => {
			appService.collectChromeActivityFromAW(arg.tpURL, arg.start, arg.end).then((res) => {
				event.sender.send('data_push_activity', {
					timerId: arg.timerId,
					windowEvent: res,
					type: 'URL'
		...
		this.electronService.ipcRenderer.on('collect_firefox_activities', (event, arg) => {
			appService.collectFirefoxActivityFromAw(arg.tpURL, arg.start, arg.end).then((res) => {
				event.sender.send('data_push_activity', {
					timerId: arg.timerId,
					windowEvent: res,
					type: 'URL'
