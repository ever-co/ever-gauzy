...
import { PrioritiesService, Store, ToastrService } from '../../../@core/services';
...
export class TaskPrioritySelectComponent extends TranslationBaseComponent
	...
	*/
	...
	getPriorities() {
		...
		this.prioritiesService.get<ITaskPriorityFindInput>({
			tenantId,
			organizationId,
			...(this.projectId
				? {
	...
	createNew = async (name: string) => {
		...
		try {
			...
			const source = this.prioritiesService.create({
				tenantId,
				organizationId,
				name,
				...(this.projectId
