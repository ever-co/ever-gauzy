...
export class PipelineDealFormComponent implements OnInit
{
  ...
  public ngOnInit(): void
  {
    ...
    this.form.patchValue( { createdByUserId: this.appStore.getValue().user?.id } );
    this.activatedRoute.params
      .subscribe( async ( { pipelineId, dealId } ) =>
      {
        ...
        if ( dealId ) {
          this.id = dealId;
          await this.dealsService.find( dealId )
            .then( ( { title, stageId } ) =>
            {
  ...
  {
    const { id, activatedRoute: relativeTo, form: { value } } = this;

    this.form.disable();
    await (this.id ? this.dealsService.update( this.id, value ) : this.dealsService.create( value ))
      .then( () => this.router.navigate( [ id ? '../..' : '..' ], { relativeTo } ) )
