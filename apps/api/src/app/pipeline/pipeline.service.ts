...
export class PipelineService extends CrudService<Pipeline>
{
  ...
  @Transaction()
  public async update( id: string | number | FindConditions<Pipeline>, partialEntity: QueryDeepPartialEntity<Pipeline>, @TransactionManager() manager: EntityManager, ...options ): Promise<UpdateResult | Pipeline>
  {
    const updatedStages = partialEntity.stages?.filter( ({ id }) => id ) || [];
    const deletedStages = await manager.find( Stage, {
      where: {
        pipelineId: id,
      },
      select: [ 'id' ],
    ...
    const createdStages = partialEntity.stages?.filter( stage => !updatedStages.includes( stage ) );
    ...
    await Promise.all( updatedStages.map( stage => manager.update( Stage, stage.id, stage ) ) );
