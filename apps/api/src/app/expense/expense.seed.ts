...
export const createExpenses = async (
    ...
    for (let index = 1; index <= 4; index++) {
        ...
        expense.vendorId = faker.random.number({ min: 10, max: 9999 });
        expense.categoryName = faker.random.words(1);
        expense.categoryId = faker.random.number({ min: 10, max: 9999 });
...
const insertExpense = async (connection: Connection, expense: Expense): Promise<void> => {
    await connection
        .createQueryBuilder()
        .insert()
        .into(Expense)
