		...
		exec(
			...
			(error, stdout) => {
				...
				if (newVersion) {
					...
					package.name = process.env.DESKTOP_TIMER_APP_NAME;
					package.productName =
					...
					package.build.productName =
						process.env.DESKTOP_TIMER_APP_DESCRIPTION;
					package.build.linux.executableName =
						process.env.DESKTOP_TIMER_APP_NAME;

					if (!isProd) {
						package.build.publish = [
							{
								provider: 'github',
								repo: process.env.DESKTOP_TIMER_APP_NAME,
								releaseType: 'prerelease',
							},
							{
								provider: 'spaces',
								name: 'ever',
								region: 'sfo3',
								path: `/${process.env.DESKTOP_TIMER_APP_NAME}-pre`,
								acl: 'public-read',
							},
						];
					} else {
						package.build.publish = [
							...
							},
							{
								...
								name: 'ever',
								region: 'sfo3',
								path: `/${process.env.DESKTOP_TIMER_APP_NAME}`,
								acl: 'public-read',
							},
						];
					}
